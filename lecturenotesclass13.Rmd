...
```{r eval = FALSE}
library(usethis)
use_git_config(user.name= "jessvchen", user.email="jess.chen@duke.edu")
```
...
What is the downside to using point estimates? Point estimates may not convey the full information contained in the sample data.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-jessvchen/manhattan.csv")
```
...
scheme outlined below using functions from `infer`.
...
get_ci(boot_med, level = 0.95)
```

We are 95% certain that the actual median lies between the interval of $2162 and $2875.
...
- Population: Mice that dies while not on treatment
- Parameter of interest: Proportion
- Sample: The mice that died while not on treatment
- Sample size: 522 vairables
...
We are 95% certain that the actual proportion of mice that died while not on treatment lies between the interval of 0.690 and 0.797.
...
visualise(boot_prop) +
  shade_ci(ci_mice_lived, color = "blue", size = 1) +
  labs(title = "Proportion of Mice That Lived While On Treatment",
       subtitle = "Zone within blue bars is 90% confidence interval",
       x = "Proportion of Mice",
       y = "Number of Mice")
```

We can be 90% confident that the actual proportion of mice that lived while on the treatment regardless of the dose lies between the interval of 0.552 and 0.651.
